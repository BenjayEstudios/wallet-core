...
if (!empty($data->id) && !empty($data->monto) && !empty($data->descripcion) && !empty($data->id_categoria) && !empty($data->estado) && !empty($data->fecha_transaccion)) {
    try {
        $query = "UPDATE tbl_transacciones 
                  SET id_categoria=:id_categoria, tipo_flujo=:tipo_flujo, titulo=:titulo, descripcion=:descripcion, monto=:monto, estado_pago=:estado_pago, fecha_transaccion=:fecha
        ...
        $fecha = htmlspecialchars(strip_tags($data->fecha_transaccion));
