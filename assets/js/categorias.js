                ...
                <div class="btn-secondary" style="display:flex; justify-content:space-between; align-items:center; padding: 15px; border-color: var(--borde); text-align:left; cursor:pointer;" onclick="abrirModalCategoria('${c.id}', '${c.nombre_categoria}', '${c.icono}', '${c.tipo_flujo}', '${catColorHex}', ${esDefault})">
                    <div style="display:flex; align-items:center; gap:10px;">
                        ...
                        <div style="display:flex; flex-direction:column;">
                            ...
                            <div style="margin-top: 4px;">${badgeCatHtml}</div>
                        </div>
                    </div>
                    <div style="display:flex; align-items:center; gap:10px;">
...
function abrirModalCategoria(id = '', nombre = '', icono = '', tipo = 'gasto', color = '#4A5568', esDefault = false) {
    if (esDefault) { mostrarToast("Las categorías por defecto no se pueden editar."); return; }
    ...
    document.getElementById('titulo-modal-cat').innerText = id === '' ? 'Nueva Categoría' : 'Editar Categoría';
    actualizarPreview();
...
async function guardarCategoria() {
    ...
    try {
        const response = await fetch(url, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(datos) });
